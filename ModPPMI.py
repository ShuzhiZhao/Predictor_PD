...
def Prediction(work_dir):
    conName_dir = work_dir+'/name_Bold.txt'
            ...
            for jj in range(np.array(FT1_data).shape[1]):
                for gg in range(np.array(FT2_data).shape[1]):
                    for ff in range(np.array(FT3_data).shape[1]):
                        ...
                        txn.put((ii+'_'+FT1_Type+str(jj)+FT2_Type+str(gg)+FT3_Type+str(ff)+'_scores').encode(), np.array(FT1_scores[0]))
    ...
    #         print(FT1_scores,FT2_scores,FT3_scores)
            ...
            for jj in range(np.array(FT1_data).shape[1]):
                for gg in range(np.array(FT2_data).shape[1]):
                    ...
                    txn.put((ii+'_'+FT1_Type+str(jj)+FT2_Type+str(gg)+'_scores').encode(), np.array(FT1_scores[0]))
...
def get3DFT(FT1_lmdb,name,dataType):
    ...
    with lmdb_env.begin() as lmdb_mod_txn:
        ...
        for idx,(key, value) in enumerate(mod_cursor.iterprev()):
            key = str(key, encoding='utf-8')                
            if "T1" in dataType :
                if '-0_score' in key and name in key :
                    score.append(np.frombuffer(value).tolist())
...
#                     print('+++++++++++ T1 '+key+' +++++++++++++++')
                ...
                if '_score' in key and name in key :
                    score.append(np.frombuffer(value).tolist())
...
#                     print('+++++++++++ Bold '+key+' +++++++++++++++')
        ...
        data.append(temp1)
    print('data shape: ',np.array(data).shape,'score shape: ',np.array(score).shape)
    lmdb_env.close()
    return data,score
...
## main program
work_dir = '/media/lhj/Momery/PD_predictDL/Data/PPMI'
